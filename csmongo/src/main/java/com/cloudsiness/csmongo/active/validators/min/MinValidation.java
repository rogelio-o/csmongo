 ...
 * @version	0.1
 */
public class MinValidation implements CsConstraintValidator<Min> {
	...
	public void initialize(String message, Long value, boolean inclusive) {
		this.message = message == null || message.isEmpty() ? Min.MESSAGE : message;
		this.value = value;
	...
	public void isValid(Object o, Function<String, CsValidationError> func, Handler<AsyncResult<CsValidationError>> handler) {
		...
		if(o instanceof BigDecimal) {
			...
			handler.handle(Future.failedFuture(new CsValidatorNotValidForAttributeType("Only BigDecimal, BigInteger, byte, short, int, long attributes are supported on Min validator.")));
			return;
		}
