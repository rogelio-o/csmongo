 ...
 * @version	0.1
 */
public class DecimalMinValidation implements CsConstraintValidator<DecimalMin> {
	...
	public void initialize(String message, BigDecimal value, boolean inclusive) {
		this.message = message == null || message.isEmpty() ? DecimalMin.MESSAGE : message;
		this.value = value;
	...
	public void isValid(Object o, Function<String, CsValidationError> func, Handler<AsyncResult<CsValidationError>> handler) {
		...
		if(o instanceof BigDecimal) {
			...
			handler.handle(Future.failedFuture(new CsValidatorNotValidForAttributeType("Only BigDecimal, CharSequence, double and float attributes are supported on DecimalMin validator.")));
			return;
		}
